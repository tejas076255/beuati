...
**Update (Phase 3I.2A):** `supabase_migrations.schema_migrations` was later confirmed directly queryable, and contains exactly one row — the original Lovable project-creation baseline (`20260814053705`). None of the ~25 migration files added since are registered there. BeautyFolio currently treats `supabase/migrations` as versioned change history. The Lovable-managed production database migration ledger is not authoritative because historical schema changes were applied directly through Lovable MCP. Do not run automated db push/migration replay until a full migration-baseline reconciliation is completed.
