...
// ---------- booking-preference presets ----------
// Stored as plain hour/day integers on the existing columns — these presets
// are a UI convenience, not a new data model.
...
// ---------- working hours (stored as JSONB on availability_settings) ----------
const WORKING_DAYS = [
  "monday",
  "tuesday",
  "wednesday",
...
function parseWorkingHours(value: unknown): DayHours[] {
  ...
  if (Array.isArray(value)) {
    for (const entry of value) {
      if (
        ...
        if ((WORKING_DAYS as readonly string[]).includes(day)) {
          byDay.set(day, {
            day: day as WorkingDay,
            available: Boolean(e["available"]),
            start: typeof e["start"] === "string" ? e["start"] : "10:00",
            end: typeof e["end"] === "string" ? e["end"] : "19:00",
          });
        }
      }
    }
  }
  return WORKING_DAYS.map((day) => byDay.get(day) ?? defaultDayHours(day));
}
...
function BlockedDatesCard() {
  ...
  const add = useMutation({
    mutationFn: () => addBlockedDateFn({ data: { blocked_date: date, reason: reason || null } }),
    onSuccess: () => {
  ...
  return (
    <Card className="border-border/70 shadow-sm">
      ...
      <CardContent className="space-y-4">
        {/* Deliberately a <div>, not a nested <form> — this card renders
            inside the page's main <form> (for the Save changes button), and
            HTML forms cannot nest. A submit button in a nested form gets
            hoisted to the outer form and triggers a full page
            navigation/reload instead of this card's own mutation. */}
        <div className="flex flex-wrap items-end gap-3">
          <label className="text-sm font-medium">
            ...
            <Input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" && date) add.mutate();
              }}
              className="mt-1"
            />
          </label>
          ...
          <Button
            ...
            onClick={() => add.mutate()}
          >
            {add.isPending ? "Adding…" : "Add blocked date"}
          </Button>
        </div>
        ...
        {blockedQuery.isLoading ? (
          ...
          <ul className="divide-y divide-border rounded-lg border border-border">
            {blockedDates.map((b: Tables<"availability_blocked_dates">) => (
...
function AvailabilityPage() {
  ...
  const hasSavedSettings = availabilityQuery.data != null;
        ...
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit(
              ...
              (errors) => {
                // handleSubmit silently no-ops on validation failure otherwise —
                // surface it so "Save changes" never appears to do nothing.
                const firstError = Object.values(errors)[0];
                toast.error(
                  firstError && "message" in firstError && typeof firstError.message === "string"
                    ? firstError.message
                    : "Please check the highlighted field before saving.",
                );
              },
            )}
          >
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Set when clients can request appointments and how you handle your availability.
                </p>
              </div>
            ...
            {/* What this feature does, in one glance */}
            <Card className="mt-4 border-border/70 bg-secondary/20 shadow-none">
              <CardContent className="p-4">
                <p className="text-sm text-muted-foreground">
                  Let clients request your availability for a specific date and service. Your
                  availability settings help you manage when and how clients can contact you.
                </p>
            ...
            {!hasSavedSettings && (
              <p className="mt-4 rounded-lg border border-dashed border-border/70 bg-secondary/20 px-4 py-3 text-xs text-muted-foreground">
                You haven't saved availability settings yet — the defaults below will be used until
                you save.
              </p>
            )}

            <div className="mt-6 space-y-6">
              {/* 1. Availability status — the ON/OFF switch */}
              <Card className="border-border/70 shadow-sm">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-base">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 rounded-lg border border-border bg-secondary/20 p-4">
                    <div>
                      ...
                      <p className="mt-1 max-w-md text-sm text-muted-foreground">
                        {acceptingBookings
                          ? "Clients can request bookings from your public portfolio."
                          : 'The "Check availability" option will be disabled or clearly marked unavailable on your public portfolio.'}
                      </p>
                    </div>
                    <div className="flex items-center gap-3">
              ...
              <Card className="border-border/70 shadow-sm">
                <CardHeader>
                  ...
                  <CardDescription>
                    How much lead time you need and how far ahead clients can plan.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      {travelAvailable && (
                        <p className="mt-1 text-xs text-muted-foreground">
                          Travel appointments available within my service areas.
                        </p>
                      )}
                    </div>
                    <FormField
              ...
              <Card className="border-border/70 shadow-sm">
                <CardHeader>
                  ...
                  <CardDescription>
                    Turn on the days you work and set your usual hours. You don't need to configure
                    every day.
                  </CardDescription>
                </CardHeader>
                <CardContent>
              ...
              <Card className="border-border/70 shadow-sm">
                <CardHeader>
                  ...
                  <CardDescription>
                    Optional context shown alongside your booking rules — e.g. "By appointment
                    only."
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          This is separate from the day-by-day hours above and can add any extra
                          context clients should know.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
